...
base = os.getcwd()
pdf_path = os.path.join(base, "pdf-sessoes", "sessao-17.pdf")
# Session 16 is in modulo-3 based on index.html MODULES array (15-22)
output_dir = os.path.join(base, "modulo-3")
img_dir = os.path.join(output_dir, "img", "sessao-16")
...
title_line = None
for line in lines:
    if line.lower().startswith('session 16:') or line.lower().startswith('sessão 16:'):
        title_line = line
        break
if title_line is None:
    ...
    for line in lines:
        if not line.startswith('Class Notes:') and not (' of ' in line and line.replace(' ', '').isdigit() is False):
            ...
            break
if title_line is None:
    title_line = "Session 16: Law as Mission"  # from actual PDF
...
pt_title = f"Sessão 16: {base_title}"
en_title = f"Session 16: {base_title}"
...
after_script = after_script.replace("exodus-s2-lang", "exodus-s16-lang")
...
for i, text in enumerate(pages_text):
    ...
    img_path = f"img/sessao-16/p{i+1}-vector.png"
...
# Write the output
output_path = os.path.join(output_dir, "sessao-16.html")
