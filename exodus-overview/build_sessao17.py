...
base = os.getcwd()
pdf_path = os.path.join(base, "pdf-sessoes", "sessao-18.pdf")
# Session 17 is in modulo-3 based on index.html MODULES array (15-22)
output_dir = os.path.join(base, "modulo-3")
img_dir = os.path.join(output_dir, "img", "sessao-17")
...
# Find the script opening after the div close
...
inner_content = template[div_open_end:div_close_idx]  # between div open and div close
between_div_and_script = template[div_close_end:script_open_idx]  # between div close and script open
after_script = template[script_open_end:]  # after script open
...
title_line = None
for line in lines:
    if line.lower().startswith('session 17:') or line.lower().startswith('sessão 17:'):
        title_line = line
        break
if title_line is None:
    ...
    for line in lines:
        if not line.startswith('Class Notes:') and not (' of ' in line and line.replace(' ', '').isdigit() is False):
            ...
            break
if title_line is None:
    title_line = "Session 17: [Title to be extracted]"  # placeholder
...
pt_title = f"Sessão 17: {base_title}"
en_title = f"Session 17: {base_title}"

# Update the head title in before_div
# Find <title> and </title> in before_div and replace the content
...
if title_open != -1 and title_close != -1:
    ...
    # If not found, just replace the whole title tag? but we assume it's there.
    pass

# Update the localStorage key in after_script
after_script = after_script.replace("exodus-s2-lang", "exodus-s17-lang")
...
for i, text in enumerate(pages_text):
    ...
    img_path = f"img/sessao-17/p{i+1}-vector.png"
...
# Write the output
output_path = os.path.join(output_dir, "sessao-17.html")
