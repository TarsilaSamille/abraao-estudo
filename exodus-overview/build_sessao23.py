...
base = os.getcwd()
pdf_path = os.path.join(base, "pdf-sessoes", "sessao-24.pdf")
# Session 23 is in modulo-4 based on index.html MODULES array (23-28)
output_dir = os.path.join(base, "modulo-4")
img_dir = os.path.join(output_dir, "img", "sessao-23")
...
title_line = None
for line in lines:
    if line.lower().startswith('session 23:') or line.lower().startswith('sessão 23:'):
        title_line = line
        break
if title_line is None:
    ...
    for line in lines:
        if not line.startswith('Class Notes:') and not (' of ' in line and line.replace(' ', '').isdigit() is False):
            ...
            break
if title_line is None:
    title_line = "Session 23: Tabernacle Furniture"  # from actual PDF
...
pt_title = f"Sessão 23: {base_title}"
en_title = f"Session 23: {base_title}"
...
after_script = after_script.replace("exodus-s2-lang", "exodus-s23-lang")
...
for i, text in enumerate(pages_text):
    ...
    img_path = f"img/sessao-23/p{i+1}-vector.png"
...
# Write the output
output_path = os.path.join(output_dir, "sessao-23.html")
