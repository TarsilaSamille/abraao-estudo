...
base = os.getcwd()
pdf_path = os.path.join(base, "pdf-sessoes", "sessao-21.pdf")
# Session 20 is in modulo-3 based on index.html MODULES array (15-22)
output_dir = os.path.join(base, "modulo-3")
img_dir = os.path.join(output_dir, "img", "sessao-20")
...
title_line = None
for line in lines:
    if line.lower().startswith('session 20:') or line.lower().startswith('sessão 20:'):
        title_line = line
        break
if title_line is None:
    ...
    for line in lines:
        if not line.startswith('Class Notes:') and not (' of ' in line and line.replace(' ', '').isdigit() is False):
            ...
            break
if title_line is None:
    title_line = "Session 20: [Title to be extracted]"  # placeholder
...
pt_title = f"Sessão 20: {base_title}"
en_title = f"Session 20: {base_title}"
...
after_script = after_script.replace("exodus-s2-lang", "exodus-s20-lang")
...
for i, text in enumerate(pages_text):
    ...
    img_path = f"img/sessao-20/p{i+1}-vector.png"
...
# Write the output
output_path = os.path.join(output_dir, "sessao-20.html")
