...
base = os.getcwd()
pdf_path = os.path.join(base, "pdf-sessoes", "sessao-13.pdf")
# Session 12 is in modulo-2 based on index.html MODULES array (7-14)
output_dir = os.path.join(base, "modulo-2")
img_dir = os.path.join(output_dir, "img", "sessao-12")
...
title_line = None
for line in lines:
    if line.lower().startswith('session 13:') or line.lower().startswith('sessão 13:'):
        title_line = line
        break
if title_line is None:
    ...
    for line in lines:
        if not line.startswith('Class Notes:') and not (' of ' in line and line.replace(' ', '').isdigit() is False):
            ...
            break
if title_line is None:
    title_line = "Session 13: Liminal Space"  # final fallback
...
pt_title = f"Sessão 12: {base_title}"
en_title = f"Session 12: {base_title}"
...
after_script = after_script.replace("exodus-s2-lang", "exodus-s12-lang")
...
for i, text in enumerate(pages_text):
    ...
    img_path = f"img/sessao-12/p{i+1}-vector.png"
...
# Write the output
output_path = os.path.join(output_dir, "sessao-12.html")
