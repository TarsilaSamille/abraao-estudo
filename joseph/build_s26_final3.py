...
if not hr_match:
    ...
    exit(1)
content_before_hr_end = div_open_end + hr_match.end()  # points after HR
...
close_match = re.search(close_pattern, template[search_start:])
if not close_match:
    print("Could not find closing div followed by script")
    # Debug: let's see what's after HR
    snippet = template[search_start:search_start+200]
    print(f"Snippet after HR: {repr(snippet)}")
    exit(1)
