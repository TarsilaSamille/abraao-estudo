...
# 6. Update title tag
# Replace <title>... </title> with Portuguese title
template = re.sub(r'<title>.*?</title>', f'<title>{portuguese_title}</title>', template, flags=re.DOTALL)

# 7. Update h1 title spans
# Pattern: <h1 class="title reveal"><span class="lang-pt">.*?</span><span class="lang-en">.*?</span></h1>
# We'll replace each span content
def replace_span_content(html, lang, new_content):
    # Find <span class="lang-pt">... </span> or lang-en
    pattern = rf'(<span class="lang-{lang}">).*?(</span>)'
    return re.sub(pattern, rf'\1{new_content}\2', html, flags=re.DOTALL)

template = replace_span_content(template, 'pt', portuguese_title)
template = replace_span_content(template, 'en', english_title)
