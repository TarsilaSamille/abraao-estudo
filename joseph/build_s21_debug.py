...
print("Index content length:", len(index_content))
print("Index first 100 chars:", repr(index_content[:100]))

# Find the MODULES array
pattern = r'const MODULES = (\\[.*?\\]);'
match = re.search(pattern, index_content, re.DOTALL)
print("MODULES match:", match)
if match:
    modules_json = match.group(1)
    modules_json = modules_json.replace("'", '"')
    print("Modules JSON string:", modules_json)
    try:
        MODULES = json.loads(modules_json)
        print("MODULES parsed:", MODULES)
...
print("Module index path:", module_index_path)
with open(module_index_path, 'r', encoding='utf-8') as f:
    module_index_content = f.read()
print("Module index content length:", len(module_index_content))
print("Module index first 200 chars:", repr(module_index_content[:200]))
...
print("SESSIONS match:", session_match)
if session_match:
    sessions_json = session_match.group(1)
    print("Sessions JSON string (raw):", repr(sessions_json))
    sessions_json = sessions_json.replace("'", '"')
    print("Sessions JSON string (after replace):", sessions_json)
    try:
        SESSIONS = json.loads(sessions_json)
        print("SESSIONS parsed:", SESSIONS)
    except json.JSONDecodeError as e:
        print("Failed to parse SESSIONS JSON:", e)
        print("Error details:", e.doc[:200] if e.doc else "No doc")
        SESSIONS = []
else:
    SESSIONS = []
    print("No SESSIONS match")
    # Let's try to see what's around the area where we expect SESSIONS
    # Find the position of 'const SESSIONS'
    pos = module_index_content.find('const SESSIONS')
    if pos != -1:
        print("Found 'const SESSIONS' at position", pos)
        print("Context:", repr(module_index_content[pos:pos+200]))
    else:
        print("'const SESSIONS' not found")
...
if session_info is None:
    ...
    print("Available sessions:", [s.get('n') for s in SESSIONS])
    exit(1)
...
doc.close()
print("PDF processed, pages:", len(pages_text))
...
print("First page title line:", repr(title_line))
...
template = template.replace("localStorage.setItem('joseph-s2-lang',l)", "localStorage.setItem('joseph-s21-lang',l)")
template = template.replace("localStorage.getItem('joseph-s2-lang')", "localStorage.getItem('joseph-s21-lang')")
