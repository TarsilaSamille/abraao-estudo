...
if start == -1:
    ...
    if end == -1:
        ...
        array_str = module_index_content[start:end+1]
        try:
            SESSIONS = json.loads(array_str)
        except json.JSONDecodeError as e:
            print(f"Failed to parse SESSIONS JSON: {e}")
            print(f"Sessions JSON string: {array_str}")
            SESSIONS = []
...
# 7. Replace the h1.title content: we will replace the inner content of the h1 tag
# Find the opening h1 tag
h1_open = '<h1 class="title reveal">'
h1_close = '</h1>'
open_idx = template.find(h1_open)
if open_idx == -1:
    print("Could not find h1 opening tag")
    exit(1)
close_idx = template.find(h1_close, open_idx)
if close_idx == -1:
    print("Could not find h1 closing tag")
    exit(1)
# The inner content is between open_idx + len(h1_open) and close_idx
new_inner = f'<span class="lang-pt">Sessão {N}: {title_pt}</span><span class="lang-en">Session {N}: {title_en}</span>'
# Rebuild the template with the new inner content
template = template[:open_idx + len(h1_open)] + new_inner + template[close_idx:]
...
print("After replacements, template length:", len(template))

# 9. Process the div content: keep up to HR, replace after HR
...
if not match:
    ...
    exit(1)
div_open_end = match.end()
print(f"div_open_end: {div_open_end}")

# Find HR after the div opening
hr_pattern = r'(<hr class=\"rule\">)'
hr_match = re.search(hr_pattern, template[div_open_end:])
if not hr_match:
    print("Could not find hr tag in inner content")
    exit(1)
content_before_hr_end = div_open_end + hr_match.end()
print(f"content_before_hr_end: {content_before_hr_end}")
print(f"Content before HR (last 100 chars): {repr(template[content_before_hr_end-100:content_before_hr_end])}")

# Now, we want to find the closing div of the main content (the one that is followed by a script tag).
# Look for the script tag in the template after content_before_hr_end.
search_area = template[content_before_hr_end:]
script_match = re.search(r'<script>', search_area)
if not script_match:
    print("Could not find script tag after HR")
    exit(1)
script_start_rel = script_match.start()  # relative to search_area
script_end_rel = script_match.end()      # relative to search_area
print(f"Found script tag at relative position {script_start_rel}-{script_end_rel}")

# Now, look backwards from the script tag to find the last closing div.
preceding = search_area[:script_start_rel]  # everything before the script tag
last_div_rel = preceding.rfind('</div>')
if last_div_rel == -1:
    print("Could not find closing div before script tag")
    exit(1)
print(f"Last closing div before script at relative position {last_div_rel}")
# The closing div string is '</div>' (length 6)
div_start_rel = last_div_rel
div_end_rel = last_div_rel + 6  # after the closing div
print(f"Closing div at relative positions {div_start_rel}-{div_end_rel}")

# Now compute absolute positions in the template:
div_close_start = content_before_hr_end + div_start_rel
script_end_abs = content_before_hr_end + script_end_rel
print(f"Script tag ends at absolute position {script_end_abs}")

# The inner content to replace is from content_before_hr_end to div_close_start (i.e., the content after HR and before the closing div).
# We'll keep the content before HR (template[0:content_before_hr_end]), then our page content, then from div_close_start to script_end_abs, then the rest after script_end_abs.
...
new_template = template[:content_before_hr_end] + page_content + template[div_close_start:script_end_abs] + template[script_end_abs:]
