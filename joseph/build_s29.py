...
print(f"PT title from SESSIONS: {title_pt}")
print(f"EN title from SESSIONS: {title_en}")

# Now, extract text from PDF (teacher's notes) for session N
pdf_path = f"{base}/joseph-teacher-notes.pdf"
doc = fitz.open(pdf_path)
pages_text = []
found_session = False
session_start_page = None

# First, find where session N starts
for i in range(doc.page_count):
    text = doc.load_page(i).get_text()
    if f"Session {N}:" in text or f"Session {N} " in text:
        found_session = True
        session_start_page = i
        break

if not found_session:
    print(f"Could not find Session {N} in teacher's notes")
    doc.close()
    exit(1)

# Extract text from session start page to the end of the document (since it's the last session)
for i in range(session_start_page, doc.page_count):
    text = doc.load_page(i).get_text()
    pages_text.append(text)

doc.close()

print(f"Extracted {len(pages_text)} pages for session {N}")

# Render diagrams (pages with >=8 drawings) as vector images at 2x scale
doc = fitz.open(pdf_path)
rendered_count = 0
for i, page in enumerate(doc):
    # Only check pages that are in our extracted range
    if session_start_page <= i < session_start_page + len(pages_text):
        drawings = page.get_drawings()
        if len(drawings) >= 8:
            pix = page.get_pixmap(matrix=fitz.Matrix(2.0, 2.0))
            # Calculate page number within the session (1-indexed)
            page_in_session = i - session_start_page + 1
            img_path = f"{img_dir}/p{page_in_session}-vector.png"
...
# Get title from PDF (first page of extracted text) for verification
first_page_text = pages_text[0]
first_page_lines = first_page_text.split('\n')
title_line = ""
for line in first_page_lines:
    if line.strip().startswith(f"Session {N}:"):
        title_line = line.strip()
        break
if not title_line:
    for line in first_page_lines:
        if line.strip() and f"Session {N}" in line:
            title_line = line.strip()
            break
if not title_line:
...
print(f"Title line from PDF: {title_line}")

# Extract the part after "Session {N}: " or similar
if title_line.startswith(f"Session {N}:"):
    prefix = f"Session {N}: "
    ...
    # Try to find session pattern
    match = re.search(f'Session {N}[:\\s]+(.*)', title_line)
    if match:
...
# But we'll use the one from SESSIONS array as the authoritative title
print(f"Using PT title: {title_pt}")
print(f"Using EN title: {title_en}")
...
template = template.replace("localStorage.setItem('joseph-s2-lang',l)", f"localStorage.setItem('joseph-s{N}-lang',l)")
template = template.replace("localStorage.getItem('joseph-s2-lang')", f"localStorage.getItem('joseph-s{N}-lang')")
