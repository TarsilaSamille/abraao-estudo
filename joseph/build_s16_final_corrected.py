...
# 6. Update title tag - replace only the content between <title> and </title>
template = re.sub(r'(<title>).*?(</title>)', rf'\1{portuguese_title}\2', template)

# 7. Update h1 tag: replace the content inside the lang-pt and lang-en spans
# Find the h1 tag and replace the span contents
def replace_h1_spans(html, pt_text, en_text):
    # Pattern to match the h1 tag and capture its inner content
    # We'll replace the content of the lang-pt and lang-en spans specifically
    html = re.sub(r'(<span class="lang-pt">).*?(</span>)', rf'\1{pt_text}\2', html)
    html = re.sub(r'(<span class="lang-en">).*?(</span>)', rf'\1{en_text}\2', html)
    return html

template = replace_h1_spans(template, portuguese_title, english_title)
