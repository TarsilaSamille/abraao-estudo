...
if start == -1:
    ...
    # Find the ending '];' (closing bracket and semicolon)
    end = module_index_content.find('];', start)
    if end == -1:
        SESSIONS = []
    else:
        # Extract the array string (including the closing bracket)
        array_str = module_index_content[start:end+1]  # end is index of ']', we want to include it
        try:
