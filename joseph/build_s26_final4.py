...
# Find the closing div that is followed by a script tag, after the HR
close_pattern = r'(</div>\\s*<script>)'
search_start = content_before_hr_end
search_area = template[search_start:]
print(f"Search area length: {len(search_area)}")
print(f"First 200 chars of search area: {repr(search_area[:200])}")
close_match = re.search(close_pattern, search_area)
if not close_match:
    print("Could not find closing div followed by script")
    # Let's try to find the script tag and see what precedes it
    script_match = re.search(r'<script>', search_area)
    if script_match:
        print(f"Found script tag at relative position {script_match.start()}")
        # Look backwards for a closing div
        preceding = search_area[:script_match.start()]
        last_div = preceding.rfind('</div>')
        if last_div != -1:
            print(f"Last closing div before script at relative position {last_div}")
            # Check what's between that div and the script
            between = preceding[last_div+6:script_match.start()]  # 6 is len('</div>')
            print(f"Between div and script: {repr(between)}")
        else:
            print("No closing div found before script.")
    else:
        print("No script tag found after HR.")
    exit(1)

div_close_start = search_start + close_match.start()   # start of '</div>'
div_close_end = search_start + close_match.end()       # after '<script>'
print(f"div_close_start: {div_close_start}, div_close_end: {div_close_end}")
print(f"Matched text: {repr(template[div_close_start:div_close_end])}")
print(f"Content before closing div (from content_before_hr_end to div_close_start): {repr(template[content_before_hr_end:div_close_start])}")

# The inner content to replace is from content_before_hr_end to div_close_start
# We'll keep the content before HR (template[0:content_before_hr_end]), then our page content, then from div_close_start to the end.
page_content = ""
...
new_template = template[:content_before_hr_end] + page_content + template[div_close_start:]
