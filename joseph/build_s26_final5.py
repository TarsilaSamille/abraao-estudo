...
div_close_end = content_before_hr_end + div_end_rel + (script_end_rel - script_start_rel)  # Actually we want to include the script tag as well.
# Wait: we want to keep from the start of the closing div to the end of the script tag.
# So we want to keep template[div_close_start : content_before_hr_end + script_end_rel]
# Because the script tag ends at content_before_hr_end + script_end_rel.
# Let's compute:
script_end_abs = content_before_hr_end + script_end_rel
print(f"Script tag ends at absolute position {script_end_abs}")
# So we will keep from div_close_start to script_end_abs.
