...
if session_match:
    ...
    print(f"Raw sessions_json: {sessions_json}")
    # Convert JavaScript object literal to JSON: add quotes around keys
    # Pattern: match a { or , followed by optional whitespace, then a word, then optional whitespace, then :
    # We'll replace the word with "word"
    sessions_json = re.sub(r'([{,]\s*)(\w+)(\s*:)', r'\1"\2"\3', sessions_json)
    print(f"After regex: {sessions_json}")
    try:
