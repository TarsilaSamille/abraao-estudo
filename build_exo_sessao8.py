import os, re, fitz
base = os.getcwd()
course = 'exodus-overview'
N = 8
pdf_path = f'{base}/{course}/pdf-sessoes/sessao-{N+1}.pdf'  # sessao-9.pdf
print(f'PDF path: {pdf_path}')
# Determine modulo from index.html SESSIONS array
...
if match:
    ...
    MODULES = eval(modules_json)  # simple as it's safe JSON-like
else:
    # fallback
    MODULES = [
...
output_dir = f'{base}/{course}/modulo-{target_module["pos"]}'
...
doc.close()
# Load template
template_path = f'{base}/{course}/modulo-1/sessao-2.html'
...
template = template.replace("localStorage.setItem('exodus-s2-lang',l)", "localStorage.setItem('exodus-s8-lang',l)")
template = template.replace("localStorage.getItem('exodus-s2-lang')", "localStorage.getItem('exodus-s8-lang')")
